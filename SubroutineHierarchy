...
public class SubroutineHierarchy extends JPanel {
	...
	private void drawWorld(Graphics2D g2) {
		...
		// TODO: Draw the content of the scene.
		 swingLine(g2,0,0,0,0);
		 
		 
		
		
	        Polygon(g2,0.0075,0.0075,1.4,-0.95);
	        Polygon(g2,0.0075,0.0075,-1.4,-0.45);
	        swingLine(g2,0.29,0.13,0,-0.7);
	        triangle(g2,Color.BLUE,0.4,1.1,0,-0.7);
	        

        Polygon(g2,0.0035,0.0035,2.7,1.9);
        Polygon(g2,0.0035,0.0035,1.3,2.1);
        swingLine(g2,0.14,0.06,2,2);
		triangle(g2,new Color(0, 100, 0),0.2,0.6,2,2);
        
		
	        Polygon(g2,0.005,0.005,-1.3,1.65);
	        Polygon(g2,0.005,0.005,-3.1,1.95);
	        swingLine(g2,0.18,0.08,-2.2,1.8);
	        triangle(g2,new Color(128, 0, 128),0.3,0.8,-2.2,1.8);
    ...
    private void swingLine(Graphics2D g2, double scalanieX, double scalanieY, double translateX, double translateY)
    {
        AffineTransform saveTransform = g2.getTransform();
        g2.setColor(Color.RED);
        g2.translate(translateX,translateY);
        g2.scale(scalanieX,scalanieY);
        g2.rotate(-0.2);
        g2.drawLine(-5,1,5,-1);
    ...
    private void triangle(Graphics2D g2, Color color, double scalanieX, double scalanieY, double translateX, double translateY)
    {
        AffineTransform saveTransform = g2.getTransform();
        g2.setColor(color);
        g2.translate(translateX,translateY);
	...
	private void Polygon(Graphics2D g2,double scalanieX, double scalanieY, double translateX, double translateY)
	{
		AffineTransform saveTransform = g2.getTransform();  
