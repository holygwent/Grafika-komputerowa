...
//funkcja przyjmuj¹ca iloœæ boków 
 void Sciany(float n)
{
    float deg = 360 / n;
    double x, y;
    // Rozpoczêcie rysowania wachlarza trójk¹tów!! Bedzie ³¹czy³ wierzcho³ki wachlarz trójk¹tów
    glBegin(GL_TRIANGLE_FAN);
    //tworzy wierzcho³ek w punkcji (0,0,0) czyli (x,y,z)
   glVertex3d(0, 0, 0);
 ...
 void Podstawa(float n)
{
    ...
    // Rozpoczêcie rysowania wachlarza trójk¹tów!! Bedzie ³¹czy³ wierzcho³ki wachlarz trójk¹tów
    glBegin(GL_TRIANGLE_FAN);
    //tworzy wierzcho³ek w punkcji (0,0,3)
    glVertex3d(0, 0, 3);
    for (float i = 1; i <= n + 1; i++)
    {
        x = cos((deg * i*M_PI)/180);
...
{
   //mno¿y bie¿¹c¹ macierz przez ogóln¹ macierz skalowania
    glScalef(2, 2, 2);
    //mno¿y bie¿¹c¹ macierz przez macierz rotacjy
    glRotatef(90, 1, 0, 0);
 
    //Ustawia kolor wed³ug modelu rgb
    glColor3f(0, 200, 155);
    //funkcja tworz¹ca n scian trójk¹tów
    Sciany(n);
  

    //Ustawia kolor wed³ug modelu rgb
    glColor3f(1, 1, 1);
    //funkcja tworz¹ca podstawe o N bokach z trójk¹tów
    Podstawa(n );
   
}

 //funkcja przyjmuje ilosc obrotów n oraz wartoœæ kolorów dla modelu rgb
void Korkociag(int n,int r,int g,int b)
{
    ...
    //zaczyna ³¹czenie wierzcho³ków lini¹
    ...
    glEnd();//koniec ³¹czenia wierzcho³ków

    
}

void draw() {
    glPushMatrix();//wrzucenie obiektu na stos transformacji (modelowanie hierachiczne)
    glRotatef(rotateZ, 0, 0, 1);   // Aplikuje rotacje dla obiektu
    glRotatef(rotateY, 0, 1, 0);     // Aplikuje rotacje dla obiektu
    glRotatef(rotateX, 1, 0, 0);     // Aplikuje rotacje dla obiektu
    ...
    glPopMatrix();//wyrzuca obiekt ze stosu transformacji
